...
MIN_DIMENSION     = 512
LSV_RADIUS        = 3        # patch radius for channel-controlled KS test
...
def combine_scores(layer_scores, re_encoded=False):
    ...
    Denominator = number of layers with score > 0, with one constraint:
    Layer D (blind spatial) is only included if at least one manifest-mode
    layer is also active and non-zero.

    RATIONALE:
    Layer D is a blind detector — it fires based on pixel statistics alone,
    with no manifest. In the absence of any manifest evidence (no markers,
    no sentinels, no payload), Layer D has no anchor. Every natural photograph
    has chromatic asymmetry (|R-G| ≠ |R-B|) that causes Layer D to fire at
    high confidence regardless of whether any embedding was done.

    Layer D is therefore corroborating evidence, not primary evidence:
    - Marked image: A=1, E=1, F=1 → manifest present → D included
    - Clean image:  A=0, E=0, F=0 → no manifest → D excluded → combined=0.0

    This preserves Layer D's value (it genuinely amplifies under compression)
    while preventing it from generating false positives on clean images.

    Manifest-mode layers: A, BC, E, F (all require embedding to fire).
    Blind layer: D (fires on pixel statistics alone).
    """
    MANIFEST_LAYERS = {"layer_a", "layer_bc", "layer_e", "layer_f"}

    candidate = {}
    for layer, result in layer_scores.items():
        score = result.get("score")
        if score is None:
            continue   # layer not evaluated
        if layer == "layer_a" and re_encoded:
            continue   # expected absence after re-encode — always exclude
        candidate[layer] = score

    # Non-zero manifest layers
    manifest_active = {k: v for k, v in candidate.items()
                       if k in MANIFEST_LAYERS and v > 0}

    # Build active set: all non-zero layers, but Layer D only if
    # at least one manifest layer is also active
    active = {}
    for k, v in candidate.items():
        if v <= 0:
            continue
        if k == "layer_d" and not manifest_active:
            continue   # no manifest anchor — exclude blind layer
        active[k] = v
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clean-dir",   "-C", default=None,
